...
public class RestAPI {
	...
	public ResponseEntity<String> addRecord(@RequestBody Record newRecord) {
		...
		//checkt eerst op correct gebruik van + en - mbt mutaties, evenals dat een (internationale) IBAN tussen de 15 en 34 karakters ligt
		...
		boolean validateEndBalance = recordAnalyzer.validateEndBalance(newRecord); 
		 ...
		 if (checkUniqueReference && validateEndBalance) { 			
			...
			return ResponseHelper.create_RESPONSE_twiceRecord("DUPLICATE_REFERENCE_INCORRECT_END_BALANCE", newRecord, HttpStatus.OK);
	...
	public List<Record> getListSuccessfulEntries() {
		return recordAnalyzer.returnList();
	}
